...
static void *
vector_address(vector *vector, int index) {
  int addr = vector->elementSize * index;
  return (char *)vector->elements + addr;
}

static void
vector_copy_item(vector *vector, int sourceIndex, int destIndex) {
...
void
vector_new(vector *vector, int elementSize, freeFunction freeFn) {
...
void
vector_item_at(vector *vector, int index, void *target) {
...
void
vector_insert_at(vector *vector, int index, void *target) {
  assert(index >= 0 && index <= vector->logicalLength);
...
void
vector_remove_at(vector *vector, int index) {
  assert(index >= 0 && index < vector->logicalLength);
